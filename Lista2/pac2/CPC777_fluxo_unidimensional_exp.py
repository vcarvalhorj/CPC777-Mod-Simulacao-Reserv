...
dr = 60.96                                                            #espassamento da malha [cm]
...
t1 = 60*60*24 #*30                                                    #[s] tempo total de análise
...
for i in range (Nt):
    ...
    for j in range (Nr):
        if j == 0:
            P_aux += [2*tau_c1*P[j+1]+(1-2*tau_c1)*P[j] - (ni*dt/(r[j]**2))*cc]                
        elif j == Nr-1:
            P_aux += [P_aux[j-1]]                                                                                  
        else:
            P_aux += [((1/r[j])*tau_c2 +tau_c1)*P[j+1] + (tau_c1-(1/r[j])*tau_c2)*P[j-1] + (1 - 2*tau_c1)*P[j]]     
